import { verifyAndClick, verify ,waitForElement, swipe, aeroplaneModeOn, aeroplaneModeOff } from '/Users/nagasubarayudu/Desktop/IOS/helpers/helper.js';
...
class RecordingPage {
    ...
    { 
        return $('//XCUIElementTypeApplication[@name="Noki-T"]/XCUIElementTypeWindow[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[2]/XCUIElementTypeOther[3]/XCUIElementTypeScrollView'); 
    }
    ...
    { 
        return $(`//XCUIElementTypeStaticText[@name="We’ve saved your conversation. It’ll sync when you’re connected again."]`); 
    }
    ...
    async recordAudioforOfflineMode() {
        await this.audioManager.AudioCommand();
        console.log("Audio started:", this.audioManager.currentAudioFile);
        await this.recordAudioforOfflineModeMT();
        await driver.pause(5000)
        await verifyAndClick(this.pauseBtn)
        await this.audioManager.pauseAudio();
        console.log("Audio paused at:", this.audioManager.pausedTime, "seconds");
        await driver.pause(20000)
        await this.playBtn.click()
        await this.audioManager.resumeAudio();
        console.log("Audio resumed:", this.audioManager.currentAudioFile);
        ...
        await driver.pause(5000);
        console.log("here we have verified that the in offline mode when we click stop button it willshould show a popup of offline conversation is saved")
        if(this.PrevEncounterRefNo.isDisplayed()){
        await this.PrevEncounterRefNo.click()
        console.log("Here her are undrgoing conversation may be s followup or the patient is visted the doctor previously")
        }
        else{
          console.log("This Encounter we are recording for this particulat patient is for the First time")
        }
   }
      ...
      async dataScanning(scrollableElement, {

...
export async function AudioCommand(command) {
  ...
  export async function swipe(direction, scrollElement) {
    ...
    export async function aeroplaneModeOff(){
      ...
      await driver.activateApp("com.apple.Preferences");
      await driver.pause(4000);
      const aeroplaneModOff = await driver.$('(//XCUIElementTypeSwitch[@value="1"])[2]');
      await verifyAndClick(aeroplaneModOff)
      await verify(aeroplaneModOff);
      await driver.pause(2000);
      await driver.activateApp("com.thinkhat.nokiTest");
      await driver.pause(4000);
      //await verifyAndClick(RecordingPage.playBtn)

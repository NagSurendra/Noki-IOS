...
export const config = {
    ...
    onAfterCommand(commandName, args, result, error) {
        ...
        if (allure && allure.getCurrentTest()) { 
            if (commandName === 'takeScreenshot') {
                ...
                // Run your script after attaching screenshot
                exec('node generateIOSreport.js', (err, stdout, stderr) => {
                    if (err) {
                        console.error('Error running generateIOSreport.js:', err);
                        return;
                    }
                    if (stderr) {
                        console.error('generateIOSreport.js stderr:', stderr);
                    }
                    console.log('generateIOSreport.js output:', stdout);
                });
            }
        } else {
  ...
  onComplete: async function (_exitCode, _config, _capabilities, _results) {
    ...
    exec(`allure open`, (err) => {
      if (err) console.error('⚠️ Failed to open Allure report automatically:', err);
    ...
    if (!fs.existsSync(reportPath)) {
      ...
      exec(`open ${reportPath}`, (err) => {
        if (err) console.error('⚠️ Failed to open HTML report automatically:', err);
